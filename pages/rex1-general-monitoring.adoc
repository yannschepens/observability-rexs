...
[NOTE.speaker]
====
Problématique : il faut lire le contenu des fichiers pour les stocker
...
[NOTE.speaker]
====
Problématique : Récuperer les logs stdout/stderr docker
...
[NOTE.speaker]
====
Problématique : pour normaliser les logs, il faut déjà les centraliser
...
[NOTE.speaker]
====
Problématique : Comment organiser/normaliser les logs ?
...
[NOTE.speaker]
====
Problématique : il faut pouvoir stocker les logs rapidement et pouvoir interagir avec.
...
[NOTE.speaker]
====
Problématique : Comment organiser le stockage des logs pour simplifier leur accès et leur maintenance ?

Solution : Des index par types et origines des logs

* Découpage par mois et années
* Format des noms d'index : <service>_<env>-<log_file>-YYYY-MM
* Une centaine d'index par mois
* Nettoyage des logs via un script en fonction du type de log
** 1 an pour les logs access par ex.
** 1 <mois pour les logs d'erreurs
** 1 mois pour les logs applicatifs

Maintenant il y a les ILM et les datastream avec la stack ELK, mais c'était pas le cas à l'éqoque.

Transition : et la dernière brique, la vis'
====
...
* Pas d'authentification (no go internet)
* Permet de filter/trier/visualiser les logs
* Permet de sauvegarder des recherches de logs
* Permet de visualiser plusieurs types de logs en même
...
[NOTE.speaker]
====
Problématique : Comment exploiter un peu plus les logs
Solution : Grafana

* Faire des graphiques à partir des logs
* Système d'alerting
* Authentification possible, donc ouverture sur internet possible
* Permet d'avoir une vision globale sur la plateforme
* Peut se brancher avec d'autres outils :
** Zabbix
** Gitlab Prometheus
** PFSense (en cours d'étude)
** ...

Transition : Et du coup, au global
====
...
[NOTE.speaker]
====
Donc on est sur une archi relativement standard mais qui clairement répond à notre besoin.

Transition : mais maintenant, ça donne quoi à l'exploit'
====